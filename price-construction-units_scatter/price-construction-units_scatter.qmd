...
Specifically, many people argue that new construction fuels increases in housing prices, as typified by the "gentrification building."
But basic principles of supply and demand should suggest that if the supply is high enough, no matter the demand, the prices shouldn't increase.
...
I then imported the price index data. I had already cleaned it up a bit in a Google Sheet, but to get all the PUMA names in full I turned the /'s and &'s to commas.
...
Is it perfect? No. But it does capture the bigger picture, which works for basic exploratory analysis.
...
I binded the tract center and tract within data together, then melted it and selected the rows only when the tract is within the PUMA.
Next I took the mean of the average units per PUMA and added the PUMA names to their IDs (which is why I kept just the names and IDs).
...
[Building Unit Census Data](https://censusreporter.org/data/table/?table=B25024&geo_ids=16000US1714000,140|16000US1714000&primary_geo_id=16000US1714000)
